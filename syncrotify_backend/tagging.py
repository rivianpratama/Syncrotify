...
MV_SEPARATOR_VISUAL = " & "
req = CachedSession("syncrotify", expire_after=3600, use_cache_dir=True)
...
def metadata_applier(tags: Tags, fixed_location: Path, exclude_tags: list[str], fallback_mv = True):
	...
	if "cover" not in exclude_tags:
		...
		# Rockbox specific processing: JPEG only, Non-Progressive
		cover_bytes = process_image_for_rockbox(cover_bytes)
...
def process_image_for_rockbox(image_bytes: bytes) -> bytes:
	"""
	Ensures the image is a non-progressive JPEG.
	"""
	try:
		...
		out = BytesIO()
		# optimize=False and progressive=False are defaults but being explicit helps
		img.save(out, format="JPEG", quality=75, optimize=False, progressive=False)
		out.seek(0)
		return out.read()
	except Exception as e:
		print(f"Failed to process image for rockbox: {e}")
		return image_bytes
...
def apply_genre_internal(file_path: Path):
	"""
	Applies genre tagging using the embedded whatlastgenre module.
	Running specifically on the single file provided.
	"""
	global _wlg_instance
	try:
		if _wlg_instance is None:
			from argparse import Namespace
			from .wlg.whatlastgenre import WhatLastGenre, Config
			from .wlg import whatlastgenre as wlg_module
			...
			# Force sources to public ones only
			if not conf.has_section('wlg'):
				conf.add_section('wlg')
			conf.set('wlg', 'sources', 'lastfm, mbrainz')
			
			# Monkeypatch ask_user
			def mock_ask_user(dapr_name, query_type, results):
				return [results[0]]
			wlg_module.ask_user = mock_ask_user

			# Initialize WhatLastGenre
			_wlg_instance = WhatLastGenre(conf)

		# Run on the file
		print(f"Applying genre tags to {file_path}")
		_wlg_instance.progress_path(str(file_path))
		
	except Exception as e:
		print(f"Genre tagging failed for {file_path}: {e}")

def get_video_id_from_file(file_path: Path) -> str | None:
	"""
	Reads the 'comments' tag from the file and extracts the YouTube Video ID.
	Expected format: https://music.youtube.com/watch?v={video_id}
	"""
	try:
		handle = MediaFile(file_path)
		if handle.comments:
			# Prioritize: If comment is exactly 11 chars (ID length) and no spaces, assume ID
			if len(handle.comments) == 11 and " " not in handle.comments:
				return handle.comments
			# Fallback: Check for URL format (Backward compatibility)
			elif "music.youtube.com/watch?v=" in handle.comments:
				return handle.comments.split("v=")[1].split("&")[0]
	except Exception:
		pass
	return None
